  ...
  context "#add" do
    context "with a string" do
      it "should send a message via the logdev" do
        subject.logdev.dev.should_receive(:write).with(/severity=WARN, pid=2025, foo/)
        subject.add(Logger::WARN) { :pid => 2025 'foo' }
      end
    end
    ...
    context "with a hash" do
      it "should send a message via the logdev" do
        ...
        subject.logdev.dev.should_receive(:write).with(/pid=2025/)
